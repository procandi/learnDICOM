module WORKLIST
  ...
  class Link
    ...
    def handle_incoming_data(path)
      ...
      # => if transfer syntaxes is ["1.2.840.10008.1.2"], then skip, beacause this is not file. @miffyer
      if file_transfer_syntaxes!=["1.2.840.10008.1.2"]
    ...
    def handle_response_cfindrsp
      ...
      data_elements = [
        ["0010,0010", "test223"], # SOP Instance UID
        ["0010,0020", "mfr0813"]
      ]
      build_data_fragment(data_elements, @presentation_context_id)
      transmit
    ...
    def interpret_command_and_data(message, file=nil)
      ...
      if info[:presentation_context_flag] == COMMAND_LAST_FRAGMENT
        ...
        if file
          ...
          # => very important! output info bin data and decode data element. @miffyer.
          p 'info bin'
          p info[:bin]
